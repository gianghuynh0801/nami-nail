...
export default function UsersPage() {
  const t = useTranslations('DashboardPages.users')
  ...
  const handleDelete = async (userId: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        alert(error.error || t('cannotDelete'))
      }
    } catch (error) {
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center gap-2">
                        ...
                        <span className="text-sm text-gray-600">
                          {user.role === 'OWNER' ? t('fullAccess') : t('permissionsCount', { count: user.permissions.length })}
                        </span>
                      </div>
                    </td>
