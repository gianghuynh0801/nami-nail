...
export default function ShiftManagementPage() {
  const t = useTranslations('DashboardPages.shiftManagement')
    ...
    const handleAssignmentChanged = (data: any) => {
      ...
      if (data.type === 'new-appointment') {
        setNotifications((prev) => [
          ...
          {
            id: Date.now(),
            message: t('newAppointment', { name: data.customerName }),
            type: 'info',
          },
        ])
        setTimeout(() => {
  ...
  if (!selectedSalonId) {
    return (
      <div className="flex items-center justify-center h-[calc(100vh-8rem)]">
        <div className="text-center">
          <p className="text-gray-500 mb-2">{t('selectBranch')}</p>
        </div>
      </div>
    )
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-4 gap-3">
        ...
        <div className="lg:col-span-3">
          <div className="mb-2 flex items-center justify-between">
            <div className="flex items-center gap-3">
              <h2 className="text-base font-semibold text-gray-900">{t('leaderboardTitle')}</h2>
              <span className="text-xs text-gray-500">
                {t('peopleCount', { count: shiftData?.staff.length || 0 })}
              </span>
            </div>
        ...
        <div className="lg:col-span-1 space-y-3">
          ...
          <div className="bg-white rounded-lg shadow p-3">
            <h3 className="font-semibold text-sm text-gray-900 mb-2">{t('overview')}</h3>
            <div className="space-y-1.5">
              <div className="flex justify-between">
                <span className="text-xs text-gray-600">{t('inProgressCount')}</span>
                <span className="text-xs font-semibold text-blue-600">
                  {shiftData?.inProgressAppointments.length || 0}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-gray-600">{t('waitingCount')}</span>
                <span className="text-xs font-semibold text-yellow-600">
                  {shiftData?.waitingQueue.length || 0}
                </span>
              </div>
              <div className="flex justify-between">
                <span className="text-xs text-gray-600">{t('availableStaff')}</span>
