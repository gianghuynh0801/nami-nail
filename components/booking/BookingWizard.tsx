...
import { X, CheckCircle2, Scissors, Users, Calendar, User, FileText, ArrowRight, ArrowLeft, Building2 } from 'lucide-react'
...
type Step = 'branch' | 'service' | 'staff' | 'datetime' | 'customer' | 'confirmation'

const steps: { id: Step; label: string; subtitle: string; icon: any }[] = [
  { id: 'branch', label: 'Branch', subtitle: 'Select branch', icon: Building2 },
  { id: 'service', label: 'Service', subtitle: 'Select service', icon: Scissors },
  { id: 'staff', label: 'Staff', subtitle: 'Select staff', icon: Users },
  { id: 'datetime', label: 'Date & Time', subtitle: 'Select time', icon: Calendar },
  { id: 'customer', label: 'Information', subtitle: 'Customer information', icon: User },
  { id: 'confirmation', label: 'Confirmation', subtitle: 'Review & book', icon: FileText },
]

export default function BookingWizard({ salon: initialSalon, services: initialServices, staff: initialStaff, salons, onClose }: BookingWizardProps) {
  const router = useRouter()
  const [currentStep, setCurrentStep] = useState<Step>(initialSalon ? 'service' : 'branch')
  const [selectedSalonId, setSelectedSalonId] = useState<string | null>(initialSalon?.id || null)
  ...
  const handleSalonSelect = async (salonId: string) => {
    ...
    try {
      const response = await fetch(`/api/booking/salon-data?salonId=${salonId}`)
      if (response.ok) {
        ...
        <div className="flex items-center justify-between p-6 border-b border-gray-200 bg-white">
          <div className="flex items-center gap-4">
            ...
            <div>
              ...
              <p className="text-sm text-gray-600">{salon?.name || 'Select a branch'}</p>
            </div>
          </div>
          {onClose && (
            <button
