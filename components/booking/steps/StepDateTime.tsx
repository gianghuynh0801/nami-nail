...
export default function StepDateTime({
  ...
  }, [selectedDate, serviceIds, staffId])
  ...
  const fetchAvailableTimes = async () => {
    if (!selectedDate || serviceIds.length === 0 || !staffId) return
    ...
    try {
      ...
      if (response.ok) {
        ...
        if (times.length > 0 && typeof times[0] === 'string') {
          ...
          setAvailableTimes(times.map((t: string) => ({ time: t, available: true })))
        } else {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h2 className="text-2xl font-bold text-gray-900 mb-2">Chọn ngày & giờ</h2>
